...
<div class="box box-primary">
    <div class="box-body">
        {{ Form::open(['action' => ['ManagerUserController@update', $data->id], 'method' => 'PUT', 'class' => 'user-edit-form user-form']) }}
            <div class="col-sm-6">
                <div class="form-group">
                    {{ Form::label('username', 'Tên đăng nhập') }}
                    {{ Form::text('username', Common::getObject($data, 'username'), ['class' => 'form-control', 'required' =>'', 'autocomplete' => 'off', 'disabled' => true]) }}
                </div>
                <div class="form-group">
                    {{ Form::label('email', 'Email') }}
                    {{ Form::email('email', Common::getObject($data, 'email'), ['class' => 'form-control', 'required' =>'', 'autocomplete' => 'off', 'disabled' => true]) }}
                </div>
                <div class="form-group">
                        ...
                        @foreach (Common::getAllCenter() as $centerId => $center)
                            <div class="form-group checkbox col-sm-6 margin0">
                                <label for="center-{{ $centerId }}">
                                    {{ Form::checkbox('center_id[]', $centerId, in_array($centerId, $centerData), ['id' => 'center-'.$centerId] ) . $center }}
                                </label>
                            </div>
                        @endforeach
                    </div>
